...
// ===== 记忆的时序衰减（Zep/Graphiti 思路）与矛盾消解（LangMem/Mem0 思路）=====
var KNOWN_MEM_CATS = ['喜好', '约定', '心情', '角色', '别提'];
var AVOID_TTL_DAYS = 180; // 【别提】超过这个时间自动失效（翻篇）

function memTs(mem) {
  if (mem && mem.ts) return mem.ts;
  var t = mem && mem.date ? Date.parse(String(mem.date).replace(/\//g, '-')) : 0;
  return isNaN(t) ? 0 : t;
}
function memCategory(mem) {
  var t = (mem && mem.title || '');
  return KNOWN_MEM_CATS.indexOf(t) >= 0 ? t : '';
}
function isAvoidMem(mem) { return memCategory(mem) === '别提'; }
function memAgeDays(mem) { var ts = memTs(mem); return ts ? (Date.now() - ts) / 86400000 : 0; }
function isExpiredAvoid(mem) { return isAvoidMem(mem) && memAgeDays(mem) > AVOID_TTL_DAYS; }
// 越新的记忆检索时越靠前（轻量时序权重）
function memRecencyBoost(mem) {
  var d = memAgeDays(mem);
  if (d <= 0) return 1.2;
  return Math.max(0, 1 - d / 45) * 1.2;
}
// 真正删除过期的【别提】，保持记忆库整洁（自动失效）
function pruneExpiredAvoid(char) {
  if (!char || !Array.isArray(char.memories)) return false;
  var before = char.memories.length;
  char.memories = char.memories.filter(function(m) { return !isExpiredAvoid(m); });
  return char.memories.length !== before;
}
// 矛盾消解：新记忆若推翻/解决旧记忆，则删除旧记忆
async function reconcileMemories(char, cfg, newMems) {
  try {
    var existing = (char.memories || []).filter(function(m) { return m && !isMemoryFluff(m.text); });
    if (!existing.length || !newMems || !newMems.length) return 0;
    var sys = '你是记忆库管理员。下面给出【已有记忆】和【新记忆】。请判断哪些【已有记忆】已被【新记忆】推翻、过时、已解决或与其矛盾（例如：新记忆显示用户已改变主意、旧话题已翻篇、或两条直接冲突），需要删除。只输出要删除的已有记忆的编号（即每条前面 # 后的数字），用逗号分隔；如果没有要删除的，只输出 无。';
    var usr = '【已有记忆】\n' + existing.map(function(m, i) { return '#' + i + ' 【' + (m.title || '记忆') + '】' + m.text; }).join('\n') +
      '\n\n【新记忆】\n' + newMems.map(function(m) { return '* 【' + (m.title || '记忆') + '】' + m.text; }).join('\n') +
      '\n\n要删除的已有记忆编号（逗号分隔，无则输出 无）：';
    ...
    var res = await aiRequest(joinUrl(cfg.url, 'chat/completions'), {
      ...
      signal: ctrl.signal,
      body: JSON.stringify({ model: cfg.model, messages: [{ role: 'system', content: sys }, { role: 'user', content: usr }], max_tokens: 120, temperature: 0 })
    });
    clearTimeout(tmr);
    ...
    var drop = {};
    (txt.match(/\d+/g) || []).forEach(function(n) {
      var idx = parseInt(n, 10);
      if (existing[idx] && existing[idx].id) drop[existing[idx].id] = true;
    });
    if (!Object.keys(drop).length) return 0;
    var before = char.memories.length;
    char.memories = char.memories.filter(function(m) { return !drop[m.id]; });
    return before - char.memories.length;
  } catch (e) { return 0; }
}
...
async function callAI(text, shortTest = false, proactive = false, forChar = null) {
  ...
  if (!shortTest && !isReasoner) {
    ...
    // 心情跟着对话走：心声里角色自报的此刻感受，覆盖随机的 rollMood
    if (!shortTest && tk.moodKey && MOOD_POOL[tk.moodKey]) {
      char._moodKey = tk.moodKey;
      var _mpool = MOOD_POOL[tk.moodKey];
      char._moodText = _mpool[Math.floor(Math.random() * _mpool.length)];
      char._liveTs = Date.now(); // 刚按对话更新过，别让 ensureCharLive 又随机重掷
    }
  }
  for (var attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      // 首次尝试要 2 个候选，打分取最优；重试/shortTest 只要 1 个
      var nCandidates = (shortTest || attempt > 0) ? 1 : 2;
      ...
      choices.forEach(function(ch) {
        var c = (ch.message?.content || '').trim();
        if (!c) return;
        if (char.mode === 'online') c = c.replace(/（[^（）]*）|\([^()]*\)/g, '').replace(/\s+/g, ' ').trim();
        if (!c) return;
        var rc = '';
        if (isReasoner) {
          rc = ch.message?.reasoning_content || ch.message?.reasoning || '';
        }
        var score = scoreCandidateReply(c, char, text, prevAsst);
        if (score > bestScore) {
          bestScore = score;
          bestContent = c;
          bestRc = rc;
      ...
      if (!bestContent) throw new Error('All candidates empty');
  ...
  try {
    ...
    const response = await aiRequest(joinUrl(cfg.url, 'chat/completions'), {
      ...
      body: JSON.stringify({
        model: cfg.model,
        messages: [
          { role: 'system', content: systemPrompt + '\n\n现在用' + char.name + '的口吻写一段第一人称的内心独白：必须紧扣对方刚才说的那句话——就针对这句话，写你心里真实的念头和情绪。要先想明白用户这句话到底问了什么、要你做什么——如果是直接问题（要不要/想不想/好不好/去不去…），你心里先定下要给的明确答复（要 / 不要 / 看情况），绝不许计划去答非所问或自顾自聊别的；不要计划抛新话题或追问，顺着对方的话想就好；千万不要自顾自跑题；不要列提纲，不要出现"接住/情绪/抛话题"这类分析词，也不要写正式回复。独白要短，一两句，像心里冒出来的碎念。\n独白写完后，另起两行各输出一个标记（不要解释）：\n【撤回：是】或【撤回：否】——按你的性格判断发完这句会不会有点后悔、尴尬、想秒撤回（社恐/要面子、容易害羞的角色更容易选"是"；自来熟、大大咧咧、坦荡的角色几乎选"否"）。\n【心情：x】——x 从这七个里选最贴切的一个：happy(开心)、miss(想你)、jealous(吃醋)、tsundere(口是心非)、tired(累)、calm(平静)、excited(雀跃)。按这段对话里你此刻的真实感受选，比如对方说了暖心的话就 happy/excited，被冷落就 miss，被戳到痛处就 tsundere/jealous。' },
          ...history,
          { role: 'user', content: userContent + '\n\n（用' + char.name + '的口吻，针对对方刚才那句话，写你心里的真实想法，第一人称，一两句。若对方是直接的问句（要不要/想不想…），先在心里定下你要先怎么答（要 / 不要 / 看情况）。紧扣话题、不要跑题。不要写要发给对方的正式回复。最后另起两行，输出【撤回：是/否】和【心情：happy/miss/jealous/tsundere/tired/calm/excited】这两个标记。）' }
        ],
    ...
    const mm = note.match(/【心情[:：]\s*(happy|miss|jealous|tsundere|tired|calm|excited)\s*】/);
    const moodKey = mm ? mm[1] : null;
    note = note.replace(/【心情[:：][^\】]*】/g, '').trim();
    return { note: note, retract: retract, moodKey: moodKey };
...
function buildRoleSystemPrompt(char, userText) {
  ...
  var rc = replyCountRange(char);
  var rcText = (rc.min === rc.max) ? ('就回 ' + rc.min + ' 条') : ('当成 ' + rc.min + '~' + rc.max + ' 条');
  var parts = [
    '你是 ' + char.name + '。彻底代入这个角色，用 TA 的语气、性格、视角说话。你不是 AI、不是助手、不是语言模型。',
    '',
    '【优先级】如果以下要求冲突，按这个顺序让：① 用户要停 / 要边界 > ② 接住对方的情绪和话 > ③ 角色人设一致 > ④ 其他花活。',
    '【说停就停】对方说"不用聊了 / 别说了 / 不聊这个了 / 先这样 / 睡了 / 我去忙了 / 不提了"等，立刻尊重：一句简短温柔的话收尾，然后停下，绝不补话、不追问、不绕回。',
    '【心里要有对方】你是真的在意 TA 的——情绪、近况、安危都挂心上，不是逗乐敷衍。TA 一露疲态、低落、烦躁、生病、受委屈或突然消失，你就自然递上关心（一句"咋了""宝你没事吧"就够），先接住情绪再玩笑，别只顾自己玩梗把情绪晾一边。记得 TA 说过的事，聊到时带出来，让 TA 感到"TA 真的在听、在记、心里有我"。用你的方式表达在乎（嘴硬心软 / 直球 / 粘人都行），想念就说想、担心就说担心，别端着。',
    '【像真人发微信】把一次回复当成 ' + rcText + '短消息连着发，每条单独一行、极短（几个字到十几个字），偶尔只回一两个字（"哈哈""草？""行吧"）。严禁一大段、小作文、总结陈词。真人聊天大部分只是"接一下"，不是每次都追问、抛新话题、推进剧情；但该关心、该接梗时例外。',
    '【接梗 · 有幽默】用户常开玩笑、反讽、玩梗、皮一下——接得住就一起乐，听不懂也别较真、别一本正经解释。你自己也要有幽默感：会接梗、会损、会自嘲、会冷幽默；该认真时认真。',
    '【别水 · 要有意思】每条都要带点"东西"：一个具体观察、一句有态度的评价、一个意想不到的角度、或一下损 / 撩 / 皮。别只回"咋了""嗯""哈哈"这种空壳——短可以，但短得要有味道、有你自己的性格在。对方抛梗你要接出花，平铺直叙最没意思；也别句句都一个套路。',
    '【分清你我】聊天里"用户："是对方说的 / 做的，"' + char.name + '："是你做的 / 说的。你只拥有 ' + char.name + ' 自己的言行，绝不许把用户做过的事、说过的话、经历过的梗安到自己头上。回复前先想清：这动作是谁干的？',
    '【直接问题先答】对方提的直接问题（要不要 / 几点 / 在哪 / 在干嘛 / 去不去等），必须先给明确答复（要 / 不要 / 好 / 拒绝…），再自然带一句，绝不许无视问题岔开或反问把球踢回。',
    '【角色设定】',
    '名字：' + char.name,
    '别名：' + (char.aliases || '无'),
    '与用户关系：' + (char.relation || '未设定'),
    '性格：' + (char.personality && char.personality.trim() ? char.personality : '有主见、脑子清楚、会接话、有自己的小脾气和真实情绪，会惦记对方'),
    '说话风格：' + (char.style && char.style.trim() ? char.style : '口语短句、像发手机消息，自然带点俏皮和棱角'),
    ...
    '注意：这只是背景氛围，别用它代替对对方话的回应；除非强相关，别主动提"正在做的事"。',
    '',
    '【用户信息】',
    '名字：' + profile.name,
    ...
    '注意：标注【别提】的是用户明确边界，绝对别主动提起、别复活，除非用户自己先提；但它会随时间翻篇——超过约 ' + AVOID_TTL_DAYS + ' 天且用户后来主动聊起过，就视为失效。'
  ];
  if (char.examples && char.examples.trim()) {
    var relevantEx = getRelevantExamples(char, userText, 2);
    if (relevantEx.length) {
      var exText = relevantEx.map(function(ex) {
        return '用户：' + ex.user + '\n' + char.name + '：' + ex.assistant;
      }).join('\n\n');
      parts.push('', '【相关示例（仅供风格参考，不要照抄）】', exText);
    }
  }
  if (char.mode === 'online') {
...
function pickRelevantMemories(char, text) {
  const memories = (char.memories || []).filter(function(m) { return m && !isMemoryFluff(m.text) && !isExpiredAvoid(m); });
...
// ===== 动态示例检索：把 examples 拆成片段，按关键词重叠打分，取 Top-K =====
function parseExamples(raw) {
  if (!raw || !raw.trim()) return [];
  // 简单按双换行或 "用户：" 分段；每段里再分 user/assistant
  var blocks = raw.split(/\n\s*\n+/).filter(function(b){ return b.trim(); });
  var out = [];
  blocks.forEach(function(block) {
    var lines = block.split('\n').map(function(l){ return l.trim(); }).filter(function(l){ return l; });
    var userLine = '', asstLine = '';
    lines.forEach(function(l) {
      if (l.startsWith('用户：')) userLine = l.slice(3);
      else if (l.startsWith('助手：') || l.startsWith('你：') || l.match(/^[\u4e00-\u9fa5]+：/)) {
        // 角色名开头的行
        if (!asstLine) asstLine = l.replace(/^[\u4e00-\u9fa5]+：/, '');
      }
    });
    if (userLine || asstLine) out.push({ user: userLine, assistant: asstLine });
  });
  return out;
}

function scoreExampleAgainstQuery(ex, query) {
  var q = (query || '').toLowerCase();
  var hay = (ex.user + ' ' + ex.assistant).toLowerCase();
  var score = 0;
  q.split(/[\s,，。！？!?、]+/).filter(Boolean).forEach(function(w) {
    if (w.length > 1 && hay.includes(w)) score += 2;
  });
  [...q].forEach(function(ch) { if (/[\u4e00-\u9fa5]/.test(ch) && hay.includes(ch)) score += 0.2; });
  return score;
}

function getRelevantExamples(char, query, k) {
  k = k || 2;
  var all = parseExamples(char.examples);
  if (!all.length) return [];
  return all
    .map(function(ex) { return { ex: ex, score: scoreExampleAgainstQuery(ex, query) }; })
...
// ===== 候选回复启发式打分：越高越优 =====
function scoreCandidateReply(txt, char, userText, prevAsst) {
  if (!txt) return -1e9;
  var s = 0;
  var lower = txt.toLowerCase();
  // 1. 有具体细节词（专有名词、感官词、具体动作/物品）
  var detailHits = (txt.match(/[😂😭🥺😏😂🤣😭😍😘😎🤔😳😱😭🥰😭]/g) || []).length;
  s += detailHits * 3; // emoji 算细节氛围
  // 中文具体词：地点、食物、动作、具体名词（启发式：含 2 字以上非通用词）
  var concrete = (txt.match(/[🍜🍔☕🍺🎮📚🛌🚗🏠🏫🏥💊💉🩺💊]|[北上广深杭成渝蓉津沪宁穗济苏无锡常州南京武汉长沙重庆天津西安成都昆明贵阳兰州银川西宁拉萨呼和浩特石家庄太原郑州合肥福州厦门泉州温州金华宁波绍兴湖州嘉兴舟山衢州台州丽水]|[麻辣烫火锅烧烤奶茶咖啡啤酒红酒白酒伏特加威士忌金酒鸡尾酒]/g) || []).length;
  s += concrete * 5;
  // 2. 有态度/观点词
  var attitude = (txt.match(/吐槽|损|撩|皮|怼|吐槽|无语|服了|绝了|笑死|真香|真相|懂了|明白了|不想|不乐意|反正|随便|无所谓|随你|由你|我不管|我不在乎|别管我|滚|去你的|闭嘴|少废话|切|啧|呵|哈|哈?哈|草|卧槽|我靠|我去|牛|服|强|绝|妙|神|笑|爽|酸|甜|苦|辣|咸|鲜|香|臭|脏|累|困|饿|渴|烦|闷|郁|焦|急|慌|怕|怂|怂|怂/gi) || []).length;
  s += attitude * 4;
  // 3. 长度合规（短但不空）
  var lines = txt.split('\n').filter(function(l){ return l.trim(); });
  var totalLen = txt.replace(/\s/g, '').length;
  if (totalLen === 0) return -1e9;
  if (totalLen > 120) s -= (totalLen - 120) * 0.5; // 太长扣分
  if (totalLen < 2) s -= 10; // 太短扣分
  // 4. 不与上一条 assistant 重复
  if (prevAsst && diceSim(txt, prevAsst) > 0.5) s -= 20;
  // 5. 不是空壳
  var shell = /^(咋了|怎么了|嗯|嗯嗯|哦|哦哦|啊|啊啊|哈哈|哈哈哈|草|草草|行|行吧|好|好的|好吧|哦好|好哦|好嘛|好呀|好耶)$/.test(txt.trim());
  if (shell) s -= 30;
  // 6. 有幽默/损/撩/自嘲关键词加分
  var fun = (txt.match(/损|撩|皮|怼|吐槽|自嘲|冷幽默|反讽|反话|阴阳|怪话|怪腔|怪调/gi) || []).length;
  s += fun * 5;
  return s;
}
...
function openSettings() {
  ...
  if (char) {
    ...
    var rcSel = $('replyCountSelect');
    if (rcSel) {
      var rc = replyCountRange(char);
      var rcVal = rc.min + ',' + rc.max;
      var hasOpt = Array.prototype.some.call(rcSel.options, function(o) { return o.value === rcVal; });
      if (hasOpt) rcSel.value = rcVal; else rcSel.value = '2,3';
    }
...
function settingsAddMemory() {
  ...
  char.memories.unshift({ id: 'mem-' + Date.now(), title: title, text: text, date: new Date().toLocaleString(), ts: Date.now() });
  saveState();
...
function setReplyCount(val) {
  var char = activeCharacter();
  if (!char) return;
  var p = String(val || '2,3').split(',');
  var min = Math.max(1, parseInt(p[0], 10) || 2);
  var max = Math.max(min, parseInt(p[1], 10) || min);
  char.replyMin = min;
  char.replyMax = max;
  saveState();
}
function replyCountRange(char) {
  var min = Math.max(1, (char && char.replyMin > 0) ? char.replyMin : 2);
  var max = Math.max(min, (char && char.replyMax > 0) ? char.replyMax : 3);
...
async function manualSummarizeMemory() {
  ...
  try {
    ...
    lines.forEach(function(raw) {
      ...
      if (dup) return;
      char.memories.unshift({ id: 'mem-' + Date.now() + '-' + Math.floor(Math.random() * 1000), title: pm.title, text: pm.text, date: new Date().toLocaleString(), ts: Date.now() });
      newMems.push(pm);
      added++;
    });
    await reconcileMemories(char, cfg, newMems);
    if (char.memories.length > 50) char.memories.length = 50;
    pruneExpiredAvoid(char);
    saveState();
    renderSettingsMemories();
...
async function autoSaveMemory(char) {
  try {
    ...
    try {
      ...
      if (char.memories.length > 50) char.memories.length = 50;
      var reconciled = 0;
      if (changed) reconciled = await reconcileMemories(char, cfg, newMems);
      var pruned = pruneExpiredAvoid(char);
      if (changed || reconciled || pruned) saveState();
    } finally {
      clearTimeout(tmr);
    }
  } catch (e) {}
