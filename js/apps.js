...
function closeApp() {
  ...
  if (_igHallTimer) { clearInterval(_igHallTimer); _igHallTimer = null; }
  if (_liveHallTimer) { clearInterval(_liveHallTimer); _liveHallTimer = null; }
  studySoundStop();
  const m = $('appModal');
...
function renderApiSettings() {
  ...
  h += '<button class="ghost-btn push-test-btn" onclick="testPush()" style="justify-content:center;display:none">发送本地测试推送</button>';
  h += '<button class="ghost-btn push-test-server-btn" onclick="testServerPush()" style="justify-content:center;display:none;margin-top:-4px">📨 立刻测试服务端推送（关掉网页后收）</button>';
...
function addMemory(charId) {
  ...
  char.memories.unshift({ id: 'mem-' + Date.now(), title: $('memoryTitle').value.trim(), text, date: new Date().toLocaleString(), ts: Date.now() });
  saveState();
