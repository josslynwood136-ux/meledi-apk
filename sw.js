  ...
  e.waitUntil(
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function (list) {
      ...
      var options = {
        ...
        tag: data.tag,
        renotify: false,
        data: { url: data.url, charId: data.charId, body: data.body },
  ...
  e.waitUntil(
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function (clientList) {
      ...
      for (var i = 0; i < clientList.length; i++) {
        var c = clientList[i];
        if ('focus' in c) {
          c.postMessage({ type: 'push-click', url: target, charId: charId, body: (e.notification && e.notification.data && e.notification.data.body) || '' });
          return c.focus();
        }
      }
      // 没打开就新开
